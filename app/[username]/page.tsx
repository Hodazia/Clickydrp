...
export default function Username(){
    ...
    // Determine viewport background style based on theme settings
    const viewportBgStyle = theme.viewportType === 'image' && theme.viewportImage
        ? { backgroundImage: `url(${theme.viewportImage})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat' }
        : theme.viewportType === 'gradient' && theme.viewportGradient
            ? { background: theme.viewportGradient }
            : { backgroundColor: theme.viewportColor || '#f5f5f5' };

    // Determine card background style
    let cardBackground = theme.cardColor || "rgba(255,255,255,0.9)";
    if (theme.cardType === 'gradient') {
      cardBackground = theme.cardGradient || cardBackground;
    } else if (theme.cardType === 'image') {
      cardBackground = `url(${theme.cardImage}) center/cover no-repeat`;
    }

    const cardStyle = {
      background: theme.cardBlur ? "rgba(255,255,255,0.6)" : cardBackground,
      backdropFilter: theme.cardBlur ? `blur(${theme.cardBlur}px)` : undefined,
      WebkitBackdropFilter: theme.cardBlur ? `blur(${theme.cardBlur}px)` : undefined,
      border: `${theme.profileBorderWidth || 0}px solid ${theme.profileBorderColor || "transparent"}`,
      // Adding a check for image type to avoid background-size issues
      ...(theme.cardType === 'image' && theme.cardImage ? { backgroundSize: 'cover', backgroundPosition: 'center' } : {})
    };
     ...
     return (
        ...
        >
            {/* Card container */}
            <div
                className="w-full max-w-md flex flex-col items-center rounded-2xl p-6 shadow-lg"
                style={cardStyle}
            >
                {/* Profile image */}
                <img
                ...
                <h1 className="mt-4 text-2xl font-bold text-gray-800">{usermeta.username}</h1>
                <p className="mt-2 text-center text-gray-600">{usermeta.description}</p>
                
                {/* Social icons */}
                {usermeta?.Social && usermeta.Social.length > 0 && (
                    <div className="mt-4 flex items-center gap-3">
                        {usermeta.Social.map((social, idx) => {
                            const key = social.platform?.toLowerCase?.() || '';
                            const IconComponent = SocialIcons[key]; // Correctly get the component function
                ...
                <div className="w-full mt-6 space-y-4">
                    {usermeta.links?.map((link, idx) => (
                        <a
                            ...
                            // Moved all styles to the a tag for simplicity
                            className="w-full flex items-center gap-3 h-12 px-3 font-medium transition-colors rounded-full shadow-md hover:scale-[1.02] text-white"
                            style={{
                        ...
                        >
                            ...
                            <div className="flex-1 truncate">{link.description || link.linkUrl}</div>
                        </a>
                    ))}
                </div>
            </div>
